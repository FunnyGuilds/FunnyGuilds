...
public final class FunnyGuildsVersion {
    ...
    private static final String NIGHTLY_VERSION_FILE_URL =
        "https://raw.githubusercontent.com/FunnyGuilds/FunnyGuilds/master/updater-nightly.txt";
    ...
    public void isNewAvailable(CommandSender sender, boolean force) {
        ...
        this.funnyGuilds.getServer()
            .getScheduler()
            .runTaskAsynchronously(this.funnyGuilds, () -> {
    ...
    private void printNewVersionAvailable(CommandSender sender, String latest, boolean isNightly) {
        ...
        FunnyGuilds.getInstance()
            .getMessageService()
            .getMessage(config -> config.newVersionAvailable)
            .receiver(sender)
            .with(formatter)
            .send();
    }
