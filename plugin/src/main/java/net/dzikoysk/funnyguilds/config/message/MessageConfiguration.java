...
public class MessageConfiguration extends OkaeriConfig implements MessageRepository {
    ...
    public SendableMessage permission = ChatHolder.message(
        "&cNie masz wystarczajacych uprawnien do uzycia tej komendy!"
    );
    ...
    public SendableMessage rankLastVictimV = ChatHolder.message(
        "&7Ostatnio zostales zabity przez tego samego gracza, punkty nie zostaja odebrane!"
    );

    public SendableMessage rankLastVictimA = ChatHolder.message(
        "&7Ostatnio zabiles tego samego gracza, punkty nie zostaja dodane!"
    );
    public SendableMessage rankLastAttackerV = ChatHolder.message(
        "&7Ostatnio zabiles tego samego gracza, punkty nie zostaja odebrane!"
    );
    public SendableMessage rankLastAttackerA = ChatHolder.message(
        "&7Ostatnio zostales zabity przez tego samego gracza, punkty nie zostaja dodane!"
    );
    public SendableMessage rankIPVictim = ChatHolder.message(
        "&7Ten gracz ma taki sam adres IP, punkty nie zostaja odjete!"
    );
    public SendableMessage rankIPAttacker = ChatHolder.message(
        "&7Ten gracz ma taki sam adres IP, punkty nie zostaja dodane!"
    );
    public SendableMessage rankMemberVictim = ChatHolder.message(
        "&7Ten gracz jest czlonkiem twojej gildii, punkty nie zostaja odebrane!"
    );
    public SendableMessage rankMemberAttacker = ChatHolder.message(
        "&7Ten gracz jest czlonkiem twojej gildii, punkty nie zostaja dodane!"
    );
    public SendableMessage rankAllyVictim = ChatHolder.message(
        "&7Ten gracz jest czlonkiem sojuszniczej gildii, punkty nie zostaja odebrane!"
    );
    public SendableMessage rankAllyAttacker = ChatHolder.message(
        "&7Ten gracz jest czlonkiem sojuszniczej gildii, punkty nie zostaja dodane!"
    );
    ...
    public SendableMessage rankDeathMessage = ChatHolder.message(
        "{ATAG}&b{ATTACKER} &7({PLUS-FORMATTED}&7) zabil {VTAG}&b{VICTIM} &7({MINUS-FORMATTED}&7) uzywajac &b{ITEM-NO-AMOUNT}"
    );

    public SendableMessage rankKillMessage = TitleHolder.message(
        "&cZabiles gracza {VICTIM}",
        "&7{PLUS-FORMATTED}",
        10,
        10,
        10
    ...
    public SendableMessage rankResetItems = ChatHolder.message(
        "&cNie masz wszystkich przedmiotow! Obecnie brakuje Ci &7{ITEM} &cz &7{ITEMS}"
    );

    @Comment("Dostępne zmienne: {LAST-RANK}, {CURRENT-RANK}")
    public SendableMessage rankResetMessage = ChatHolder.message(
        "&7Zresetowales swoj ranking z poziomu &c{LAST-RANK} &7do poziomu &c{CURRENT-RANK}&7."
    );

    @Comment("Dostępne zmienne: {ITEM}, {ITEMS}")
    public SendableMessage statsResetItems = ChatHolder.message(
        "&cNie masz wszystkich przedmiotow! Obecnie brakuje Ci &7{ITEM} &cz &7{ITEMS}"
    );
    ...
    public SendableMessage regionExplode = ChatHolder.message(
        "&cBudowanie na terenie gildii zablokowane na czas &4{TIME} sekund&c!"
    );

    @Comment("Dostępne zmienne: {TIME}")
    public SendableMessage regionExplodeInteract = ChatHolder.message(
        "&cNie mozna budowac jeszcze przez &4{TIME} sekund&c!"
    );
    ...
    public SendableMessage regionExplosionHasProtection = ChatHolder.message(
        "&cEksplozja nie spowodowala zniszczen na terenie gildii, poniewaz jest ona chroniona!"
    );
    ...
    public SendableMessage notificationIntruderEnterGuildRegion = SendableMessage.builder()
        ...
        .addHolders(
            BossBarHolder.builder("&7Gracz &c{PLAYER} &7wkroczyl na teren &cTwojej &7gildii!")
    ...
    public SendableMessage notificationEnterGuildRegion = SendableMessage.builder()
        ...
        .addHolders(
            BossBarHolder.builder("&7Wkroczyles na teren gildii &c{TAG}&7!")
                .color(BossBar.Color.RED)
                .addFlag(BossBar.Flag.CREATE_WORLD_FOG)
                .stay(15 * 20)
                .clearOtherBars(true)
    ...
    public SendableMessage notificationLeaveGuildRegion = SendableMessage.builder()
        ...
        .addHolders(
            BossBarHolder.builder("&7Opusciles teren gildii &c{TAG}&7!")
    ...
    public SendableMessage broadcastCreate = ChatHolder.message(
        "&a{PLAYER} &7zalozyl gildie o nazwie &a{GUILD} &7i tagu &a{TAG}&7!"
    );
    ...
    public SendableMessage broadcastBan = ChatHolder.message(
        "&7Gildia &c{TAG}&7 zostala zbanowana za &c{REASON}&7, gratulacje!"
    );
    ...
    public SendableMessage broadcastValidity = ChatHolder.message(
        "&7Gildia &b{TAG} &7wygasla&b! &7Jej baza znajdowala sie na x: &b{X} &7y: &b{Y} &7z: &b{Z}&7!"
    );
    ...
    @Comment(
        "Dostępne zmienne: {PLAYER}, {GUILD}, {TAG}, {POINTS}, {POINTS-FORMAT}, {KILLS}, {DEATHS}, {ASSISTS}, {LOGOUTS}, {KDR}, {KDA}, {RANK}"
    )
    ...
    @Comment(
        "Dostępne zmienne: {GUILD}, {TAG}, {OWNER}, {DEPUTIES}, {MEMBERS}, {MEMBERS-ONLINE}, {MEMBERS-ALL}, {REGION-SIZE}, {POINTS}, {POINTS-FORMAT}, {KILLS}, {DEATHS}, {ASSISTS}, {LOGOUTS}, {KDR}, {KDA}, {ALLIES}, {ALLIES-TAGS}, {ENEMIES}, {ENEMIES-TAGS}, {RANK}, {VALIDITY}, {LIVES}, {LIVES-SYMBOL}, {LIVES-SYMBOL-ALL}, {GUILD-PROTECTION}"
    )
    ...
    public SendableMessage generalAllyPvpDisabled = ChatHolder.message(
        "&cPVP pomiedzy sojuszami jest wylaczone w konfiguracji!"
    );
    ...
    @Comment("Dostępne zmienne: {TIME}")
    public SendableMessage escapeStartedUser = ChatHolder.message(
        "&aDobrze, jesli nikt ci nie przeszkodzi - za {TIME} sekund uda ci sie uciec!"
    );

    @Comment("Dostępne zmienne: {TIME}, {X}, {Y}, {Z}, {PLAYER}")
    public SendableMessage escapeStartedOpponents = ChatHolder.message(
        "&cGracz {PLAYER} probuje uciec z terenu twojej gildii! ({X}  {Y}  {Z})"
    );
    ...
    @Comment("Dostępne zmienne: {PLAYER}")
    public SendableMessage escapeSuccessfulOpponents = ChatHolder.message(
        "&cGraczowi {PLAYER} udalo sie uciec z terenu twojej gildii!"
    );

    public SendableMessage escapeNoUserGuild = ChatHolder.message("&cNie masz gildii do ktorej moglbys uciekac!");
    public SendableMessage escapeNoNeedToRun = ChatHolder.message(
        "&cNie znajdujesz sie na terenie zadnej gildii, po co uciekac?"
    );
    public SendableMessage escapeOnYourRegion = ChatHolder.message(
        "&cZnajdujesz sie na terenie wlasnej gildii, dokad chcesz uciekac?"
    );
    ...
    @Comment("Dostępne zmienne: {LENGTH}")
    public SendableMessage createNameLength = ChatHolder.message(
        "&cNazwa nie moze byc dluzsza niz &c{LENGTH} litery&7!"
    );

    @Comment("Dostępne zmienne: {LENGTH}")
    public SendableMessage createTagMinLength = ChatHolder.message(
        "&7Tag nie moze byc krotszy niz &c{LENGTH} litery&7!"
    );

    @Comment("Dostępne zmienne: {LENGTH}")
    public SendableMessage createNameMinLength = ChatHolder.message(
        "&cNazwa nie moze byc krotsza niz &c{LENGTH} litery&7!"
    );
    ...
    public SendableMessage createSpawn = ChatHolder.message(
        "&7Jestes zbyt blisko spawnu! Minimalna odleglosc to &c{DISTANCE}"
    );

    public SendableMessage createIsNear = ChatHolder.message(
        "&cW poblizu znajduje sie jakas gildia, poszukaj innego miejsca!"
    );

    @Comment("Dostępne zmienne: {POINTS}, {POINTS-FORMAT}, {REQUIRED}, {REQUIRED-FORMAT}")
    public SendableMessage createRank = ChatHolder.message(
        "&cAby zalozyc gildie, wymagane jest przynajmniej &7{REQUIRED} &cpunktow."
    );

    @Comment("Dostępne zmienne: {ITEM}, {ITEMS}")
    public SendableMessage createItems = ChatHolder.message(
        "&cNie masz wszystkich przedmiotow! Obecnie brakuje Ci &7{ITEM} &cz &7{ITEMS}&c. Najedz na przedmiot, aby dowiedziec sie wiecej"
    );

    @Comment("Dostępne zmienne: {EXP}")
    public SendableMessage createExperience = ChatHolder.message(
        "&cNie posiadasz wymaganego doswiadczenia do zalozenia gildii: &7{EXP}"
    );

    @Comment("Dostępne zmienne: {MONEY}")
    public SendableMessage createMoney = ChatHolder.message(
        "&cNie posiadasz wymaganej ilosci pieniedzy do zalozenia gildii: &7{MONEY}"
    );

    public SendableMessage withdrawError = ChatHolder.message(
        "&cNie udalo sie pobrac pieniedzy z twojego konta z powodu: &7{ERROR}"
    );

    @Comment("Dostępne zmienne: {PLAYER}, {GUILD}, {TAG}")
    public SendableMessage createGuild = ChatHolder.message("&7Zalozono gildie o nazwie &a{GUILD} &7i tagu &a{TAG}&7!");

    public SendableMessage createGuildCouldNotPasteSchematic = ChatHolder.message(
        "&cWystapil blad podczas tworzenia terenu gildii, zglos sie do administracji."
    );

    @Comment("Dostępne zmienne: {BORDER-MIN-DISTANCE}")
    public SendableMessage createNotEnoughDistanceFromBorder = ChatHolder.message(
        "&cJestes zbyt blisko granicy mapy aby zalozyc gildie! (Minimalna odleglosc: {BORDER-MIN-DISTANCE})"
    );

    @Comment("")
    @Comment("<------- Delete Guild Messages -------> #")
    public SendableMessage deleteConfirm = ChatHolder.message(
        "&7Aby potwierdzic usuniecie gildii, wpisz: &c/potwierdz"
    );
    ...
    public SendableMessage inviteAmount = ChatHolder.message(
        "&7Osiagnieto juz &cmaksymalna &7liczbe czlonkow w gildii! (&c{AMOUNT}&7)"
    );

    public SendableMessage inviteAmountJoin = ChatHolder.message(
        "&7Ta gildia osiagnela juz &cmaksymalna &7liczbe czlonkow! (&c{AMOUNT}&7)"
    );
    public SendableMessage inviteAllyAmount = ChatHolder.message(
        "&7Osiagnieto juz &cmaksymalna &7liczbe sojuszy miedzygildyjnych! (&c{AMOUNT}&7)"
    );

    @Comment("Dostępne zmienne: {AMOUNT}, {GUILD}, {TAG}")
    public SendableMessage inviteAllyTargetAmount = ChatHolder.message(
        "&7Gildia {TAG} posiada juz maksymalna liczbe sojuszy! (&c{AMOUNT}&7)"
    );
    ...
    public SendableMessage inviteCancelledToInvited = ChatHolder.message(
        "&7Zaproszenie do gildii &c{GUILD} &7zostalo wycofane!"
    );
    ...
    public SendableMessage inviteRangeToBig = ChatHolder.message(
        "&cPodany zasięg jest zbyt duży! (Maksymalnie: {MAX_RANGE})"
    );

    @Comment("Dostępne zmienne: {MAX_RANGE}")
    public SendableMessage inviteAllArgumentIsNotNumber = ChatHolder.message(
        "&cPodany zasięg nie jest poprawną liczbą!"
    );
    ...
    @Comment("Dostępne zmienne: {ITEM}, {ITEMS}")
    public SendableMessage joinItems = ChatHolder.message(
        "&cNie masz wszystkich przedmiotow! Obecnie brakuje Ci &7{ITEM} &cz &7{ITEMS}"
    );
    ...
    public SendableMessage enlargeIsNear = ChatHolder.message(
        "&cW poblizu znajduje sie jakas gildia, nie mozesz powiekszyc terenu!"
    );

    @Comment("Dostępne zmienne: {ITEM}")
    public SendableMessage enlargeItem = ChatHolder.message(
        "&7Nie masz wystarczajacej liczby przedmiotow! Potrzebujesz &c{ITEM}"
    );

    @Comment("Dostępne zmienne: {SIZE}, {LEVEL}")
    public SendableMessage enlargeDone = ChatHolder.message(
        "&7Teren &aTwojej &7gildii zostal powiekszony i jego wielkosc wynosi teraz &a{SIZE} &7(poz.&a{LEVEL}&7)"
    );

    @Comment("")
    @Comment("<------- Base Messages -------> #")
    public SendableMessage baseTeleportationDisabled = ChatHolder.message(
        "&cTeleportacja do baz gildyjnych nie jest dostepna"
    );
    ...
    @Comment("Dostępne zmienne: {ITEM}, {ITEMS}")
    public SendableMessage baseItems = ChatHolder.message(
        "&cNie masz wszystkich przedmiotow! Obecnie brakuje Ci &7{ITEM} &cz &7{ITEMS}"
    );
    ...
    public SendableMessage enemyAlly = ChatHolder.message(
        "&cNie mozesz rozpoczac wojny z ta gildia poniewaz jestescie sojusznikami!"
    );
    public SendableMessage enemyAlready = ChatHolder.message("&cProwadzisz juz wojne z ta gildia!");

    @Comment("Dostępne zmienne: {AMOUNT}")
    public SendableMessage enemyMaxAmount = ChatHolder.message(
        "&7Osiagnieto juz &cmaksymalna &7liczbe wojen miedzygildyjnych! (&c{AMOUNT}&7)"
    );

    @Comment("Dostępne zmienne: {AMOUNT}, {GUILD}, {TAG}")
    public SendableMessage enemyMaxTargetAmount = ChatHolder.message(
        "&7Gildia {TAG} posiada juz maksymalna liczbe wojen! (&c{AMOUNT}&7)"
    );
    ...
    public SendableMessage allyHasNotInvitation = ChatHolder.message(
        "&7Aby zaprosic gildie do sojuszy wpisz &c/sojusz [tag]"
    );
    ...
    @Comment("Dostępne zmienne: {GUILD}, {TAG}")
    public SendableMessage allyToInvited = ChatHolder.message(
        "&7Otrzymano zaproszenie do sojuszu od gildii &a{GUILD}&7!"
    );
    ...
    @Comment("Dostępne zmienne: {GUILD}, {TAG}")
    public SendableMessage breakAllyExists = ChatHolder.message(
        "&7Twoja gildia nie posiada sojuszu z gildia (&c{TAG}&7, &c{GUILD}&7)!"
    );
    ...
    @Comment("Dostępne zmienne: {ITEM}")
    public SendableMessage validityItems = ChatHolder.message(
        "&7Nie masz wystarczajacej liczby przedmiotow! Potrzebujesz &c{ITEM}"
    );
    ...
    public SendableMessage warAttacker = ChatHolder.message(
        "&7Twoja gildia pozbawila gildie &4{ATTACKED} &7z &41 zycia&7!"
    );

    @Comment("Dostępne zmienne: {ATTACKER}")
    public SendableMessage warAttacked = ChatHolder.message(
        "&7Twoja gildia stracila &41 zycie &7przez &4{ATTACKER}&7!"
    );

    @Comment("Dostępne zmienne: {LOSER}")
    public SendableMessage warWin = ChatHolder.message(
        "&7Twoja gildia &apodbila &7gildie &a{LOSER}&7! Zyskujecie &c1 zycie&7!"
    );

    @Comment("Dostępne zmienne: {WINNER}")
    public SendableMessage warLose = ChatHolder.message(
        "&7Twoja gildia &4przegrala &7wojne z gildia &4{WINNER}&7! &4Gildia zostaje zniszona&7!"
    );
    ...
    public SendableMessage tntInfo = ChatHolder.message(
        "&7TNT na teranach gildii działa od {PROTECTION_END} do {PROTECTION_START}"
    );
    ...
    public SendableMessage adminGuildOwner = ChatHolder.message(
        "&cTen gracz jest zalozycielem gildii, nie mozna go wyrzucic!"
    );
    ...
    public SendableMessage adminPointsChanged = ChatHolder.message(
        "&aUstawiono &7{POINTS} &apunktow dla gracza &7{PLAYER}"
    );

    public SendableMessage adminNoKillsGiven = ChatHolder.message("&cPodaj liczbe zabojstw!");

    @Comment("Dostępne zmienne: {PLAYER}, {KILLS}")
    public SendableMessage adminKillsChanged = ChatHolder.message(
        "&aUstawiono &7{KILLS} &azabojstw dla gracza &7{PLAYER}"
    );

    public SendableMessage adminNoDeathsGiven = ChatHolder.message("&cPodaj liczbe zgonow!");

    @Comment("Dostępne zmienne: {PLAYER}, {DEATHS}")
    public SendableMessage adminDeathsChanged = ChatHolder.message(
        "&aUstawiono &7{DEATHS} &azgonow dla gracza &7{PLAYER}"
    );
    ...
    public SendableMessage adminLogoutsChanged = ChatHolder.message(
        "&aUstawiono &7{LOGOUTS} &awylogowań dla gracza &7{PLAYER}"
    );

    public SendableMessage adminNoAssistsGiven = ChatHolder.message("&cPodaj liczbę asyst!");

    @Comment("Dostępne zmienne: {PLAYER}, {ASSISTS}")
    public SendableMessage adminAssistsChanged = ChatHolder.message(
        "&aUstawiono &7{ASSISTS} &aasyst dla gracza &7{PLAYER}"
    );
    ...
    public SendableMessage adminLivesChanged = ChatHolder.message(
        "&aUstawiono &7{LIVES} &azyc dla gildii &7{GUILD}&a!"
    );

    @Comment("Dostępne zmienne: {GUILD}")
    public SendableMessage adminGuildRelocated = ChatHolder.message("&aPrzeniesiono teren gildii &7{GUILD}&a!");

    public SendableMessage adminNoValidityTimeGiven = ChatHolder.message(
        "&cPodaj czas o jaki ma byc przedluzona waznosc gildii!"
    );

    @Comment("Dostępne zmienne: {GUILD}, {VALIDITY}")
    public SendableMessage adminNewValidity = ChatHolder.message(
        "&aPrzedluzono waznosc gildii &a{GUILD} &7do &a{VALIDITY}&7!"
    );
    ...
    public SendableMessage adminNoProtectionDateGive = ChatHolder.message(
        "&cPodaj date ochrony dla gildii! (W formacie: yyyy/mm/dd hh:mm:ss)"
    );
    public SendableMessage adminInvalidProtectionDate = ChatHolder.message(
        "&cTo nie jest poprawna data! Poprawny format to: yyyy/mm/dd hh:mm:ss"
    );
    public SendableMessage adminProtectionSetSuccessfully = ChatHolder.message(
        "&aPomyslnie ustawiono ochrone dla gildii &7{TAG} &ado &7{DATE}"
    );

    public SendableMessage adminGuildHasNoHome = ChatHolder.message("&cGildia gracza nie ma ustawionej bazy!");

    @Comment("Dostępne zmienne: {ADMIN}")
    public SendableMessage adminTeleportedToBase = ChatHolder.message(
        "&aAdmin &7{ADMIN} &ateleportowal cie do bazy gildii!"
    );

    @Comment("Dostępne zmienne: {PLAYER}")
    public SendableMessage adminTargetTeleportedToBase = ChatHolder.message(
        "&aGracz &7{PLAYER} &azostal teleportowany do bazy gildii!"
    );
    ...
    public SendableMessage funnybinFileNotOpened = ChatHolder.message(
        "&cPodany plik: {FILE} nie mogl byc otworzony (szczegoly w konsoli)"
    );
    public SendableMessage funnybinFileNotSent = ChatHolder.message(
        "&cPodany plik: {FILE} nie mogl byc wyslany (szczegoly w konsoli)"
    );
    ...
    public SendableMessage reloadWarn = ChatHolder.message(
        "&cDziałanie pluginu FunnyGuilds po reloadzie moze byc zaburzone, zalecane jest przeprowadzenie restartu serwera!"
    );
