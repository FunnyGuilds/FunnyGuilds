...
public class GuildPlaceholdersService extends StaticPlaceholdersService<Guild, GuildPlaceholders> {
    ...
    public static final BasicPlaceholders<Pair<String, Guild>> GUILD_MEMBERS_COLOR_CONTEXT = new BasicPlaceholders<
        Pair<String, Guild>
    ...
    public String format(@Nullable Object entity, String text, Guild guild) {
        ...
        text = GUILD_MEMBERS_COLOR_CONTEXT.toVariablesFormatter(
            Pair.of(ChatUtils.getLastColorBefore(text, "{MEMBERS}"), guild)
        ).format(text);
        return text;
    }
    ...
    public static GuildPlaceholders createGuildPlaceholders(FunnyGuilds plugin) {
        ...
        return new GuildPlaceholders()
            .property(
                "owner",
                guild -> guild.getOwner().getName(),
                entity -> messages.get(entity, config -> config.gOwnerNoValue)
            )
            .property(
                "deputies",
                (entity, guild) ->
                    JOIN_OR_DEFAULT.apply(
                        Entity.names(guild.getDeputies()),
                        messages.get(entity, config -> config.gDeputiesNoValue)
                    ),
            ...
            .property(
                ...
                guild -> {
                    ...
                    if (lives <= pluginConfiguration.warLives) {
                        return (
                            StringUtils.repeated(lives, pluginConfiguration.livesRepeatingSymbol.full.getValue()) +
                            StringUtils.repeated(
                                pluginConfiguration.warLives - lives,
                                pluginConfiguration.livesRepeatingSymbol.empty.getValue()
                            )
                        );
                    } else {
                        return (
                            StringUtils.repeated(
                                ...
                                pluginConfiguration.livesRepeatingSymbol.full.getValue()
                            ) +
            ...
            .property(
                "lives-symbol-all",
                guild ->
                    StringUtils.repeated(guild.getLives(), pluginConfiguration.livesRepeatingSymbol.full.getValue()),
                entity -> messages.get(entity, config -> config.livesNoValue)
            )
            .rankProperty(
                "position",
                (entity, guild, rank) ->
                    rankManager.isRankedGuild(guild)
            ...
            // TODO total-points -> points (I know this will break up backwards compatibility so
            ...
            .rankProperty(
                "points-format",
                (entity, guild, rank) ->
                    FunnyFormatter.format(
                        NumberRange.inRangeToString(rank.getAveragePoints(), pluginConfiguration.pointsFormat),
                        "{POINTS}",
                        guild.getRank().getAveragePoints()
                    ),
                entity ->
                    FunnyFormatter.format(
                        NumberRange.inRangeToString(0, pluginConfiguration.pointsFormat),
                        "{POINTS}",
                        0
                    )
            )
    ...
    public static GuildPlaceholders createAlliesEnemiesPlaceholders(FunnyGuilds plugin) {
        ...
        return new GuildPlaceholders()
            .property(
                ...
                (entity, guild) ->
                    JOIN_OR_DEFAULT.apply(
                        Entity.names(guild.getAllies()),
                        messages.get(entity, config -> config.alliesNoValue)
                    ),
            ...
            .property(
                ...
                (entity, guild) ->
                    JOIN_OR_DEFAULT.apply(
                        GuildUtils.getTags(guild.getAllies()),
                        messages.get(entity, config -> config.alliesNoValue)
                    ),
                entity -> messages.get(entity, config -> config.alliesNoValue)
            )
            .property(
                ...
                (entity, guild) ->
                    JOIN_OR_DEFAULT.apply(
                        Entity.names(guild.getEnemies()),
                        messages.get(entity, config -> config.enemiesNoValue)
                    ),
            ...
            .property(
                ...
                (entity, guild) ->
                    JOIN_OR_DEFAULT.apply(
                        GuildUtils.getTags(guild.getEnemies()),
                        messages.get(entity, config -> config.enemiesNoValue)
                    ),
                entity -> messages.get(entity, config -> config.enemiesNoValue)
            );
