...
public class TntProtection extends AbstractFunnyListener {
    ...
    public void onExplode(EntityExplodeEvent event) {
        ...
        if (this.config.tntProtection.time.enabled) {
            ...
            event
                .blockList()
                .removeIf(block -> this.regionManager.findRegionAtLocation(block.getLocation()).isPresent());
        }
    }
    ...
    public void blockBuildingOnGuildRegionOnExplosion(EntityExplodeEvent event) {
        Set<Player> players = this.regionManager.findRegionAtLocation(event.getLocation())
            ...
            .filterNot(
                guild ->
                    this.config.warTntProtection && !this.config.regionExplodeBlockProtected && !guild.canBeAttacked()
            )
            ...
            .filterNot(user ->
                this.informationMessageCooldowns.cooldown(user.getUUID(), this.config.infoPlayerCooldown)
            )
