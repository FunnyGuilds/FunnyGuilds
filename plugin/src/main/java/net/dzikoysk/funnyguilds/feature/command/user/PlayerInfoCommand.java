...
public final class PlayerInfoCommand extends AbstractFunnyCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        User user = when(this.userManager.findByName(name, this.config.playerLookupIgnorecase), config ->
            config.generalNotPlayedBefore
        );
    ...
    public void sendInfoMessage(
        Function<MessageConfiguration, Sendable> baseMessage,
        User infoUser,
        CommandSender messageTarget
    ) {
        ...
        this.messageService.getMessage(baseMessage)
            ...
            .with(CommandSender.class, receiver -> {
                ...
                if (infoUser.hasGuild()) {
                    ...
                    guildFormatter.register(
                        "{GUILD}",
                        this.messageService.<String>get(receiver, config -> config.gNameNoValue)
                    );
                    guildFormatter.register(
                        "{TAG}",
                        this.messageService.<String>get(receiver, config -> config.gTagNoValue)
                    );
                }
