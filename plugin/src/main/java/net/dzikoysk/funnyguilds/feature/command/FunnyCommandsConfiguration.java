...
public final class FunnyCommandsConfiguration {
    ...
    private static final class CommandComponents {
        ...
        private CommandComponents command(
            String name,
            CommandsConfiguration.FunnyCommand configuration,
            Class<?> command
        ) {
            if (configuration.enabled) {
                this.placeholders.put(this.group + "." + name + ".name", key -> configuration.name);
                this.placeholders.put(this.group + "." + name + ".aliases", key ->
                    Joiner.on(", ").join(configuration.aliases).toString()
                );
