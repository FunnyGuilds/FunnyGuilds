...
public final class LeaveCommand extends AbstractFunnyCommand {
    ...
    public void execute(Player player, @IsMember User user, Guild guild) {
        when (user.isOwner(), this.messageConfig.leaveIsOwner);
        ...
        player.sendMessage(formatter.format(this.messageConfig.leaveToUser));
        Bukkit.broadcastMessage(formatter.format(this.messageConfig.broadcastLeave));
    }
