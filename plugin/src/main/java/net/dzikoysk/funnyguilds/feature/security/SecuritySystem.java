...
public final class SecuritySystem {
    ...
    private static void scan(Player player, Guild guild) {
        GuildConfiguration guildConfiguration = FunnyGuilds.getInstance().getGuildConfiguration();
        if (!guildConfiguration.isRegionsEnabled()) {
            return;
        }

        if (!guildConfiguration.securitySystem.enabled) {
            return;
        }
        ...
        FunnyBox funnyBox = guildConfiguration.region.heart.createEntityType != null
