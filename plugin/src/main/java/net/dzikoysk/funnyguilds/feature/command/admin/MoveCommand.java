...
public final class MoveCommand extends AbstractFunnyCommand {
    ...
    public void execute(Player player, User admin, String[] args) {
        ...
        when(
            distance > LocationUtils.flatDistance(player.getWorld().getSpawnLocation(), location),
        ...
        Region region = guild
            ...
            .peek(peekRegion -> {
                if (heartConfig.createEntityType != null) {
                    ...
                    heartConfig.createMaterial != null && heartConfig.createMaterial.getFirst() != Material.AIR
                ) {
                    peekRegion
                        .getHeartBlock()
        ...
        if (heartConfig.createCenterSphere) {
            SpaceUtils.sphere(location, 3, 3, false, true, 0)
                .stream()
