...
public class UserPlaceholdersService extends StaticPlaceholdersService<User, UserPlaceholders> {
    ...
    public static UserPlaceholders createUserPlaceholders(FunnyGuilds plugin) {
        ...
        return new UserPlaceholders()
            ...
            .rankProperty("points-format", (UserRank rank) ->
                FunnyFormatter.format(
    ...
    public static UserPlaceholders createPlayerPlaceholders(FunnyGuilds plugin) {
        ...
        return new UserPlaceholders()
            .playerOptionProperty("world", playerOption ->
                playerOption.map(Player::getWorld).map(World::getName).orElseGet("")
            )
            .playerOptionProperty("online", playerOption ->
                playerOption
            ...
            .playerOptionProperty("vault-money", playerOption ->
                playerOption
    ...
    private static List<String> getWorldGuardRegionNames(Player player) {
        ...
        List<String> regionNames = HookManager.WORLD_GUARD.map(worldGuard ->
            worldGuard.getRegionNames(location)
        ).orNull();
