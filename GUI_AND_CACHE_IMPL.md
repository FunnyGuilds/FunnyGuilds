...
- Globalna `guild-item-library` zawiera **wszystkie** itemy — zarówno wymagane do założenia gildii (`diamonds`, `founders-sword` itp.) jak i itemy GUI (`item-x`, `item-t`, `item-s`, `item-c`) — jeden system, jedno miejsce definicji
- Każdy item: `material`, `name`, `lore`, `enchants`, `flags`, `custom-model-data`, `skull-owner`, `armor-color` (wszystko MiniMessage)
- Sety referencjonują itemy przez klucz z ilością: `diamonds: 64` — żadnego `ref:`, żadnych inline itemów w setach
...
- Sekcja `gui:` zawiera: `enabled`, `title`, `rows`, `pattern` (tokeny referencjonują klucze z library), `item-display` (has-enough / missing), `per-set` z `library-overrides`
...
**`PatternPane`** — wzór z `items.yml` trafia dosłownie do `PatternPane`. Każdy token (`{item-x}`, `{toggle:item-t}` itp.) jest kluczem z `guild-item-library`. IF rozdziela tokeny na pozycje slotów, zero własnego parsowania numerów.

**`OutlinePane`** — ramka GUI jedną linijką zamiast ręcznego wypełniania slotów w pętli. Owijamy cały `ChestGui` outlinepane'em z `item-x` z library, zanim nałożymy `PatternPane`.
...
**`ToggleButton`** — token `{toggle:item-t}` w patternie. Klucz `item-t` w library ma dwa stany (`all` / `missing-only`) każdy z własnym `material`, `name`, `lore`. Przełącza widok między "wszystkie itemy z setu" a "tylko brakujące".
...
**`{toggle:item-t}`** — przełącznik widoku all/missing, dwa stany z library.

**`{close:item-c}`** — `onClick → gui.close()`

**`{item-x}`** — filler, definicja w library, nadpisywalny per-set przez `per-set.{set}.library-overrides.item-x`.
...
# =======================================
# Wszystkie itemy — zarówno wymagane do gildii jak i GUI — definiowane tu.
guild-item-library:

  # == ITEMY WYMAGANE DO ZAŁOŻENIA GILDII ==
  ...
  # == ITEMY GUI ==
  # Używane w patternie przez klucz — ten sam system co wyżej.
  ...
  item-t:                   # toggle — dwa stany
    all:
      material: LIME_DYE
      name: '<green><bold>Pokaż wszystkie'
      lore:
        - '<gray>Kliknij aby pokazać tylko brakujące itemy'
    missing-only:
      material: RED_DYE
      name: '<red><bold>Tylko brakujące'
      lore:
        - '<gray>Kliknij aby pokazać wszystkie itemy'
...
gui:
  ...
  # Tokeny patternu:
  #   {klucz}            - item z guild-item-library
  #   {item-1}..{item-N} - kolejne itemy z setu (auto)
  #   {toggle:klucz}     - two-state toggle, klucz z library
  #   {summary:klucz}    - summary status, klucz z library
  #   {close:klucz}      - zamknij, klucz z library
  pattern:
  ...
  # Nadpisania per-set — można nadpisać tytuł i dowolny item z library
  per-set:
    vip:
