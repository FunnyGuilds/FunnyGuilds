...
- **Content:** plain Markdown / MDX in `src/content/docs/`. To edit a page, just edit
  its `.md` file — Polish pages live directly in `src/content/docs/`, English ones in
  `src/content/docs/en/`.
...
real 3D scene with [Three.js](https://threejs.org/): `src/lib/terrain-data.ts` generates
the deterministic (seeded) layout as plain data (territories, river, height tiles, tree/
cactus scatter), and `src/lib/isomap-scene.ts` turns that into a `THREE.Scene` (instanced
boxes for terrain, `CSS2DRenderer` labels) mounted client-side onto `#map-root` via a
`<script>` in `Landing.astro`. Terrain is real geometry with real height, so a normal
depth buffer + back-face culling decides what's visible, with almost no manual occlusion
logic to maintain — the one exception is water's edge walls (see the comment on
`waterWalls` in `terrain-data.ts`), needed because this camera's fixed angle only ever
shows a block's own +x/+z face. The map needs no localization (it only renders the
four territory names, which are proper nouns); all copy and the in-map tablist/scoreboard
HUD mockup strings live in `src/lib/landing-content.ts`, keyed by locale. Being seeded, the
map is stable across builds; tweak the constants at the top of `terrain-data.ts`
(territory layout, river shape, elevation noise) to iterate on it. An earlier hand-rolled
SVG version of the renderer is kept for reference at `src/lib/isomap.legacy-svg.ts` (no
...
| Task | Where |
|---|---|
| Add / edit a Polish page | `src/content/docs/<slug>.md` |
| Add / edit an English page | `src/content/docs/en/<slug>.md` (keep the same `<slug>` as the Polish page) |
...
| The isometric map (layout/terrain data) | `src/lib/terrain-data.ts` |
| The isometric map (3D scene / rendering) | `src/lib/isomap-scene.ts` |
