...
public class FlatPatcher {
	...
	public void patch(){
		...
		if(g || r){
			...
			if(g && guildsList != null) tg   += guildsList.length;
			if(r && regionsList != null) tr  += regionsList.length;
			
			todo = tg + tr;
			FunnyGuilds.update(todo + " files found ...");
			
			FunnyGuilds.update("Updating files ...");
			if(g) guilds.renameTo(Flat.GUILDS);
			if(r) regions.renameTo(Flat.REGIONS);
			
			guildsList = guilds.listFiles();
			regionsList = regions.listFiles();
			
			if(guildsList == null || guildsList.length == 0) IOUtils.delete(guilds);
			if(regionsList == null || regionsList.length == 0)  IOUtils.delete(regions);
